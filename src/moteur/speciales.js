...
export function zoneSpeciale(ctx, i, c) {
  const g = ctx.grille, gr = ctx.etat.gravite, [x, y] = coord(g, i), r = ctx.bus.reduire('rayonSpeciale', c.rayon ?? 1, ctx, { type: c.speciale });
...
function activer(ctx, i, activations) {
  ...
  if (!c || c.type !== 'element' || c.element.type === 'relique') return;
...
export function resoudre(ctx, initiale) {
  const g = ctx.grille, file = [initiale], declenchees = new Set(initiale.ignoreSpeciales ?? []);
  ...
  while (file.length) {
    ...
    for (const i of activations) {
      const c = g.cellules[i];
      if (!c || c.type !== 'element' || c.element.type === 'relique') continue;
